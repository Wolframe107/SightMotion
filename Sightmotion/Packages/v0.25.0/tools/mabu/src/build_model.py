...
class BuildSpec(object):
    ...
    def _parse(self, spec, no_defaults, ignore_missing):
        ...
        def _report_unknown_segment(seg):
            if spec == seg:
                prefix = "unknown target segment '{}';\n".format(spec)
            else:
                prefix = "unknown target segment '{1}' in build target '{0}';\n".format(spec, seg)

            error_msg = prefix + \
                "run 'mabu --help-target' to see all valid target segments\n" \
                "(for a custom configuration, be sure --path=... or MABUPATH point to {0}.config)"\
                .format(seg)
        ...
        def _no_duplicate(seg, cur, type):
            ...
            # see if we hit an unknown config already;
            # assume the unknown one was a platform
            if self._config and self._config not in build_env.configs():
                # complain about the previously-ignored segment
                _report_unknown_segment(self._config)
            else:
...
def dump_build_objects(configs=False, targets=False, toolchains=False, archs=False):
    ...
    if targets:
        s += 'Platforms: ' + ', '.join(sort(list(Platform.names()))) + '\n'
