...
from config import DEFAULT_PACKAGE_BINARY_DIR, BUILT_PACKAGE_EXT, MANIFEST_FILE, SIGN_CERT
...
def _verify_certificate(cert_file, allow_expired):
    try:
        from OpenSSL import crypto
    except ImportError:
        diags.warn("installation problem: cannot load OpenSSL library, skipping signing certificate validation.")
        return
    ...
    try:
        ...
        try:
            ...
            # always an error (check before sign-file gives a confusing error)
            diags.error("Failed to read certificate ({}); verify it is correctly generated: {}".format(cert_file, e))

        if cert.has_expired():
            (allow_expired and diags.warn or diags.error)(
                "This certificate ({}) has expired.  Visit developer.magicleap.com to update it!".format(cert_file))

    except crypto.Error as e:
        diags.warn("failed to read certificate:".format(e.args))
        for arg in e.args:
...
def layout_or_package_project(opts, project):
    ...
    if mlcert:
        ...
        # verify
        _verify_certificate(pkgflags.cert_file, pkgflags.allow_expired_certificate)
    ...
    if mlcert:
        ...
        lines = ["Missing a developer certificate ('-s ...')."] + \
                textwrap.wrap(
                    ...
                    "For more information, see https://developer.magicleap.com/learn/guides/application-signing.",
                    width=78)
        diags.error("\n".join(lines))
...
def resign_package(opts, output_package_path):
    ...
    _verify_certificate(pkgflags.cert_file, opts.allow_expired_certificates)
